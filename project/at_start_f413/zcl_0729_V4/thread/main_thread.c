...
QueueHandle_t  xQueueMotorControlMessage;
...
//		BLE_Reset();//���籣��		
		while( MCU_at_cmd_analysis(AT_DEVICE_START_HINT)!=AT_DEVICE_START_HINT)//�ȴ������ɹ�
		{
			vTaskDelay(1);  //1ms
			#ifdef WDT_ENABLE
			xEventGroupSetBits(WDTEventGroup, BLE_TASK_EVENT_BIT);
...
{ 
	...
	{			
			...
			switch(BLE_state)
			{
				case BLE_MASTER_CONFIG:		
					...
					xEventGroupSetBits(WDTEventGroup, BLE_TASK_EVENT_BIT);						
						...
						{
							BLE_state =	BLE_POWER_OFF;//����BLE�ر�״̬							
							cmdN=AT_NULL;
						}
						else 
						{
							BLE_state =	BLE_SCAN_STATUS;//����ɨ��״̬
				...
				case BLE_SCAN_STATUS:							
						...
						if(cmdN!=AT_SCAN)
					{					
						if(timeOut>2200)//2.5s����ط�
						{
							timeOut=0;							
							BLE_transmit(AT_SCAN,TYPE_AT_HANDLE,NULL);
						}							
						if(cmdN!=AT_NULL&&ble_package.Len!=0) ble_package.Len=0;	
						if(cmdN==UNVARNISHED_BLE_DATA_REV)	//��δ���׶Ͽ�
						{
							#ifdef WDT_ENABLE
							xEventGroupSetBits(WDTEventGroup, BLE_TASK_EVENT_BIT);
							vTaskDelay(BLE_COMMUNICATION_GAP_TIME_MS);  //1ms	
							xEventGroupSetBits(WDTEventGroup, BLE_TASK_EVENT_BIT);						
							#endif
							BLE_state =	BLE_CONNECT_MANAGE;//��������״̬		
				...
				case BLE_CONNECT_MANAGE: 
						...
						{		
							...
							BLE_state=BLE_APPLICATION;//����Ӧ�����ݴ���״̬	
						}					
					   else 
						{
							vTaskDelay(5);  //10ms
							...
							#endif	
							timeOut+=5;			
							if(timeOut>2000||cmdN==CONNECTED_TIMEOUT)//5s����ط�//��ʱ�ط�	
							{						
								timeOut=0;									
								BLE_state =	BLE_SCAN_STATUS;//����ɨ��״̬							
								cmdN=AT_NULL;							
						...
						break;
				case BLE_APPLICATION://͸��ģʽ	
						cmdN=MCU_at_cmd_analysis(UNVARNISHED_BLE_DATA_REV);
					 if(cmdN==UNVARNISHED_BLE_DATA_REV)//�յ�Ӧ������
					{						//250ms��ظ���׼���ݰ�
          ...
          else if(cmdN==DISCONNECTED_HINT)//�Ͽ�����
					{
							if(sys_param_un.device_param.BLE_Connect==BLE_CONNECT_TRX)
							{								
...
//								BLE_app_data_tx_buf[7]=ble_device_info_list[0].mac[16];
							...
							{
							if(xTaskGetTickCount()>heatTime+250)//250ms��������
							{								
								BLE_app_data_tx_buf[0]=0x96;
								BLE_app_data_tx_buf[1]=(uint8_t)sys_param_un.device_param.ref_tine;									
								BLE_app_data_tx_buf[2]=GC_depth_vlaue(0, 1);//read	;
...
{ 	
	...
	unsigned  short int  rec_gc_rate;  //��ǰrate
	static 	int  GC_depth=30;//��ʼֵ
	...
	{		
		...
		if(apexGC_Flag==1)//����
		{
			...
			if((rec_gc_rate+5)<sys_param_un.device_param.gc_ref_rate||rec_gc_rate>5+sys_param_un.device_param.gc_ref_rate)  sys_param_un.device_param.gc_ref_rate=rec_gc_rate;//���²ο�ֵ	
			else 
			{				
...
//			DEBUG_PRINTF("ref-rate%d\r\n", sys_param_un.device_param.gc_ref_rate);	
		...
		else if(apexGC_Flag==2)//����У׼
		{
			...
			{				
				...
				sys_param_un.device_param.empty_rate=rec_gc_rate;//����ο�ֵ
				if(sys_param_un.device_param.empty_rate<sys_param_un.device_param.gc_ref_rate+150) sys_param_un.device_param.empty_rate=1000 ;//1000;//ʹ��Ĭ��ֵ
				gc_list_init((int)sys_param_un.device_param.empty_rate,(int)(sys_param_un.device_param.gc_ref_rate-10));
			}				
		//too low ,error value ������
			GC_PWM_SwitchTimeManage(xTaskGetTickCount(),0);//restart	
       ...
       if(get_gc_state() == RESET)
			{
				...
				{							
					...
					else //if(GC_Frequency==400)
					{
						...
						apex_frequency_set(GC_Frequency);	
						//handle
						if(GC_400_AdcValue==0) GC_400_AdcValue=1;// 6k�迹 GC_400_AdcValue=1100;
...
{ 
	static unsigned char  buzzerOp=BUZZER_MODE_POWER_ON_HINT_VOL;//������ʾ��
	...
	{		
		...
		buzzerOp=BUZZER_MODE_MUTE;	//�����ֵ			
		xQueueReceive(xQueueBeepMode, &buzzerOp, 0);//��ȡ��ֵ				
